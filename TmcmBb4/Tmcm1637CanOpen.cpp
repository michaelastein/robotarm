...
//!************************************************************************
//! Define the allowed state transitions (CiA-402)
//!
//! @returns: nothing
//!************************************************************************
void Tmcm1637CanOpen::defineAllowedStateTransitions()
{
    mAlowedStateTransitionsVec.clear();
    AllowedStateTransitions allowed;

    allowed.nextStates.clear();
    allowed.currentState = MOTION_CONTROLLER_STATE_UNKNOWN;
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCH_ON_DISABLED );
    mAlowedStateTransitionsVec.push_back( allowed );

    allowed.nextStates.clear();
    allowed.currentState = MOTION_CONTROLLER_STATE_SWITCH_ON_DISABLED;
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_READY_TO_SWITCH_ON );
    mAlowedStateTransitionsVec.push_back( allowed );

    allowed.nextStates.clear();
    allowed.currentState = MOTION_CONTROLLER_STATE_READY_TO_SWITCH_ON;
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCH_ON_DISABLED );
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCHED_ON );
    mAlowedStateTransitionsVec.push_back( allowed );

    allowed.nextStates.clear();
    allowed.currentState = MOTION_CONTROLLER_STATE_SWITCHED_ON;
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_OPERATION_ENABLED );
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_READY_TO_SWITCH_ON );
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCH_ON_DISABLED );
    mAlowedStateTransitionsVec.push_back( allowed );

    allowed.nextStates.clear();
    allowed.currentState = MOTION_CONTROLLER_STATE_OPERATION_ENABLED;
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCHED_ON );
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_READY_TO_SWITCH_ON );
    allowed.nextStates.push_back( MOTION_CONTROLLER_STATE_SWITCH_ON_DISABLED );
    mAlowedStateTransitionsVec.push_back( allowed );
...
//!************************************************************************
bool Tmcm1637CanOpen::goToHomePosition()
{
    MotionControllerState controllerState = MOTION_CONTROLLER_STATE_UNKNOWN;
    bool status = getState( controllerState );

    if( status )
    {
        status = ( MOTION_CONTROLLER_STATE_OPERATION_ENABLED == controllerState );
    }

    int32_t encodedHomePosition = getEncodedTargetPosition( mHomePosition );

    if( status )
    {
        status = setTargetPosition( encodedHomePosition );
    }
    ...
    if( status )
    {
        controlWord |= CONTROL_WORD_NEW_SET_POINT;
        status = setControlWord( controlWord );
    }

    if( status )
    {
        status = setState( MOTION_CONTROLLER_STATE_OPERATION_ENABLED );
    }
...
//! @returns: nothing
//!************************************************************************
void Tmcm1637CanOpen::setHomePosition
    (
    const int32_t aPosition //!< position
    )
{
    mHomePosition = getEncodedTargetPosition( aPosition );
}
    ...
    ) const
{
    ...
    switch( aMode )
    {
        ...
        //case OPERATING_MODE_CYC_SYNC_POSITION:
        //case OPERATING_MODE_CYC_SYNC_VELOCITY:
        //case OPERATING_MODE_CYC_SYNC_TORQUE:
            status = true;
            break;
