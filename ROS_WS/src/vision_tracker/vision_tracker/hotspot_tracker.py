...
class VisionTracker(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Vision tracker started (normalized output mode)")
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if meas is not None and conf >= CONF_THRESHOLD:
            ...
            # ---------------- NORMALIZATION (OPTION C CORE) ----------------
            # convert pixel → [-1, 1] centered coordinates
            nx = (x - (w * 0.5)) / (w * 0.5)
            ny = (y - (h * 0.5)) / (h * 0.5)
